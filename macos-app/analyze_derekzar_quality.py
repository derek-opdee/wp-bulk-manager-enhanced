#!/usr/bin/env python3
"""
Comprehensive content quality analysis for derekzar.com
Checks for:
...
def main():
    ...
    # Initialize manager and connect to derekzar
    manager = WPBulkManager()
    
    # Try to find derekzar in existing sites
    sites = manager.get_sites()
    derekzar_site = None
    
    for site in sites:
        if 'derekzar' in site['url'].lower():
            derekzar_site = site
            break
    
    if not derekzar_site:
        print("❌ derekzar.com not found in sites.")
        print("\nTo add derekzar.com, you need a WordPress Application Password.")
        ...
        print("4. Run 'python setup_derekzar.py' to add the site")
        print("\nAlternatively, you can manually add it using wpbm_cli.py")
        return
    
    # Connect to the site
    print(f"\n📡 Connecting to {derekzar_site['url']}...")
    if not manager.set_active_site(derekzar_site['id']):
        print("❌ Failed to connect to derekzar.com")
        return
    ...
    # Get all pages
    print("\n📄 Fetching all pages...")
    pages = manager.api.get_all_content('pages')
